        ...
        export let GARDEN_GRASS_COLOR=0x77767B;
        export let GARDEN_GRASS_BRIGHTNESS=1.00;
        ...
        export const GARDEN_PAVEMENT_GRADIENT={
          entrance:0xFFFFFF,
          middle:0xDEDDDA,
          end:0xCCCBCA,
          entranceBlendMeters:3.0,
          finalBlendMeters:5.0
        };

        function applyGardenPavementGradientToGeometry(__gardenCtx, geo, zMin){
          const THREE=__gardenCtx.THREE;
          const p=geo?.attributes?.position;
          if(!p) return;

          const entranceColor=new THREE.Color(GARDEN_PAVEMENT_GRADIENT.entrance);
          const middleColor=new THREE.Color(GARDEN_PAVEMENT_GRADIENT.middle);
          const endColor=new THREE.Color(GARDEN_PAVEMENT_GRADIENT.end);

          const colors=new Float32Array(p.count*3);
          const first=Math.max(.1,GARDEN_PAVEMENT_GRADIENT.entranceBlendMeters);
          const second=Math.max(.1,GARDEN_PAVEMENT_GRADIENT.finalBlendMeters);

          for(let i=0;i<p.count;i++){
            const forwardDistance=Math.max(0,p.getZ(i)-zMin);

            // Same front-to-back gradient as before.
            // Only add a SMALL spatial side bias: farther from the entrance center
            // means slightly further along the same color progression.
            const centerX=
              (GARDEN_PAVEMENT_PERIMETER.xMin+
               GARDEN_PAVEMENT_PERIMETER.xMax)*.5;
            const sideDistance=Math.abs(p.getX(i)-centerX);
            const sideBias=Math.min(1.35,sideDistance*.035);
            const distance=forwardDistance+sideBias;

            let c;

            if(distance<=first){
              const raw=THREE.MathUtils.clamp(distance/first,0,1);
              const t=raw*raw*(3-2*raw);
              c=entranceColor.clone().lerp(middleColor,t);
            }else{
              const raw=THREE.MathUtils.clamp((distance-first)/second,0,1);
              const t=raw*raw*(3-2*raw);
              c=middleColor.clone().lerp(endColor,t);
            }
        ...
        export function getGardenPavementGradientEndHex(){
          return `#${GARDEN_PAVEMENT_GRADIENT.end
            .toString(16)
            .padStart(6,"0")
            .toUpperCase()}`;
        }
        ...
        export function initGardenRuntimeState(THREE,scene){
          ...
          inward:.08,
            edit:{
            left:{x:-1.90,z:1.30,length:0.00},
            right:{x:1.70,z:1.30,length:0.00},
            back:{x:1.90,z:3.50,length:-0.10}
          }
        };
        ...
        function mergeFenceBoxParts(__gardenCtx,parts){
          const THREE=__gardenCtx.THREE;
          const positions=[];
          const normals=[];
          const uvs=[];
          ...
          for(const part of parts){
            ...
            const matrix=new THREE.Matrix4().compose(
              new THREE.Vector3(part.pos.x,part.pos.y,part.pos.z),
            ...
            for(let i=0;i<p.count;i++){
              tmpPos.fromBufferAttribute(p,i).applyMatrix4(matrix);
              positions.push(tmpPos.x,tmpPos.y,tmpPos.z);
              tmpNormal.fromBufferAttribute(n,i)
            ...
            if(index){
              ...
              for(let i=0;i<p.count;i++) indices.push(vertexOffset+i);
            }
        ...
        export function createProceduralFenceSource(__gardenCtx){
          const THREE=__gardenCtx.THREE;
          const W=.608536;
          const H=6.337368;
          const L=9.849748;
          ...
          const post=.34;
          parts.push(
            {pos:{x:0,y:0,z:-L*.5+post*.5},size:{x:W,y:H,z:post}},
            {pos:{x:0,y:0,z: L*.5-post*.5},size:{x:W,y:H,z:post}}
          );
          for(const y of [-H*.44,0,H*.44]){
            parts.push({
              pos:{x:0,y,z:0},
              size:{x:W*.72,y:.24,z:L-post*1.5}
            });
          }
          const innerCount=12;
          for(let i=1;i<=innerCount;i++){
            const z=-L*.5+(L/(innerCount+1))*i;
            parts.push({
              pos:{x:0,y:0,z},
              size:{x:W*.42,y:H*.83,z:.105}
            });
          }
          const geometry=mergeFenceBoxParts(__gardenCtx,parts);
          ...
          mesh.name="procedural_fence_mesh";
          mesh.castShadow=false;
          mesh.receiveShadow=false;
          mesh.frustumCulled=true;
          const root=new THREE.Group();
          root.name="procedural_fence_source";
          root.add(mesh);
          root.updateMatrixWorld(true);
          return root;
        }

        export function initGardenFenceSource(__gardenCtx){
          gardenFenceSource=createProceduralFenceSource(__gardenCtx);
        ...
        export function rebuildGardenConcreteSkeletonFromPerimeter(__gardenCtx){
          ...
          concreteMatGarden.color.setHex(0xFFFFFF);
          concreteMatGarden.vertexColors=true;
          concreteMatGarden.userData.gardenPavementGradient=true;
          ...
          const lawnWidth=12.0000; 
          const lawnDepth=11.5000; 
          const lawnZ=82.4390;     
          ...
          applyGardenPavementGradientToGeometry(__gardenCtx,geo,zMin);
        ...
        export function buildCeremonyGarden(__gardenCtx){
          ...
          applyGardenPavementGradientToGeometry(__gardenCtx,pavementGeo,zMin);
        ...
        // =====================================================================
        // GARDEN-OWNED MUSEUM VISUALS
        // Geometry/material placement belongs here; main only orchestrates.
        // =====================================================================
        export const DISPLAY_CASE_WOOD_COLOR={
          color:0xCDAB8F,
          enabled:true
        };

        export function ensureDisplayCaseWoodenLegs(__gardenCtx,caseObj){
          if(!caseObj) return;
          const THREE=__gardenCtx.THREE;
          const old=caseObj.parent?.getObjectByName(
            `${caseObj.name}_display_case_dark_base`
          );
          if(old) old.parent?.remove(old);
          caseObj.updateMatrixWorld(true);
          const box=new THREE.Box3().setFromObject(caseObj);
          if(box.isEmpty()) return;
          const size=box.getSize(new THREE.Vector3());
          const center=box.getCenter(new THREE.Vector3());
          const parent=caseObj.parent;
          if(!parent) return;
          const h=DISPLAY_CASE_DARK_BASE.height;
          const baseMat=new THREE.MeshStandardMaterial({
            color:DISPLAY_CASE_DARK_BASE.color,
            roughness:.82,
            metalness:.01
          });
          const base=new THREE.Mesh(
            new THREE.BoxGeometry(
              Math.max(.20,size.x-DISPLAY_CASE_DARK_BASE.insetX*2),
              h,
              Math.max(.20,size.z-DISPLAY_CASE_DARK_BASE.insetZ*2)
            ),
            baseMat
          );
          base.name=`${caseObj.name}_display_case_dark_base`;
          const worldP=new THREE.Vector3(
            center.x,
            box.min.y-h*.5,
            center.z
          );
          base.position.copy(parent.worldToLocal(worldP.clone()));
        ...
        function isDisplayCaseWoodMaterial(__gardenCtx,mesh,mat){
          if(!mesh || !mat || !mat.color) return false;
          if(mat.transparent || (Number.isFinite(mat.opacity) && mat.opacity<.92)) return false;
          const token=`${mesh.name||""} ${mat.name||""}`.toLowerCase();
          if(/glass|window|crystal|metal|steel|chrome|cloth|fabric|redcloth/.test(token)) return false;
          if(/wood|wooden|timber|frame|base|plinth|brown/.test(token)) return true;
          if(mat.userData.displayCaseOriginalColor===undefined){
            mat.userData.displayCaseOriginalColor=mat.color.getHex();
          }
          const THREE=__gardenCtx.THREE;
          const c=new THREE.Color(mat.userData.displayCaseOriginalColor);
          const max=Math.max(c.r,c.g,c.b);
          const min=Math.min(c.r,c.g,c.b);
          const brownish=(c.r>c.b*1.18 && c.g>c.b*1.05 && c.r>=c.g*.88 && (max-min)>.055);
          const notStrongMetal=!(Number.isFinite(mat.metalness) && mat.metalness>.62);
          return brownish && notStrongMetal;
        }

        function forEachDisplayCaseWoodMaterial(__gardenCtx,callback){
          const museum=__gardenCtx.GARDEN_MUSEUM || GARDEN_MUSEUM;
          if(!museum?.editable) return;
          const seen=new Set();
          for(const [key,root] of museum.editable){
            if(!key.endsWith(":case") || !root) continue;
            root.traverse(mesh=>{
              if(!mesh?.isMesh || !mesh.material) return;
              const mats=Array.isArray(mesh.material)?mesh.material:[mesh.material];
              for(const mat of mats){
                if(!mat || seen.has(mat.uuid)) continue;
                if(mat.userData.displayCaseOriginalColor===undefined && mat.color){
                  mat.userData.displayCaseOriginalColor=mat.color.getHex();
                }
                if(!isDisplayCaseWoodMaterial(__gardenCtx,mesh,mat)) continue;
                seen.add(mat.uuid);
                callback(mat,mesh);
              }
            });
        ...
        export function applyDisplayCaseWoodColor(__gardenCtx){
          let count=0;
          forEachDisplayCaseWoodMaterial(__gardenCtx,mat=>{
            if(DISPLAY_CASE_WOOD_COLOR.enabled){
              mat.color.setHex(DISPLAY_CASE_WOOD_COLOR.color);
            }else if(mat.userData.displayCaseOriginalColor!==undefined){
              mat.color.setHex(mat.userData.displayCaseOriginalColor);
            }
            mat.needsUpdate=true;
            count++;
          });
          const read=document.getElementById("displayCaseWoodColorRead");
          if(read){
            read.textContent=
              `DISPLAY CASE WOOD · ALL CASES\n`+
              `COLOR #${DISPLAY_CASE_WOOD_COLOR.color.toString(16).padStart(6,"0").toUpperCase()}\n`+
              `WOOD MATERIALS FOUND ${count}`;
          }
          return count;
        }

        export function removeDisplayCaseRedCloth(__gardenCtx,caseRoot){
          if(!caseRoot) return;
          const remove=[];
          caseRoot.traverse(mesh=>{
            if(!mesh?.isMesh) return;
            const mats=Array.isArray(mesh.material)?mesh.material:[mesh.material];
            const isRedCloth=/M_RedCloth|redcloth|red_cloth/i.test(String(mesh.name||"")) ||
              mats.some(m=>/M_RedCloth|redcloth|red_cloth/i.test(String(m?.name||"")));
            if(isRedCloth) remove.push(mesh);
          });
        ...
        export function fitArtworkIntoCase(__gardenCtx,art,caseRoot){
          if(!art || !caseRoot) return;
          const THREE=__gardenCtx.THREE;
          caseRoot.updateMatrixWorld(true);
          ...
          let artBox=new THREE.Box3().setFromObject(art);
          let artSize=artBox.getSize(new THREE.Vector3());
          const targetW=Math.max(.15,caseSize.x*.42);
          const targetD=Math.max(.15,caseSize.z*.42);
          const targetH=Math.max(.20,caseSize.y*.42);
          const factor=Math.min(
            targetW/Math.max(artSize.x,.001),
            targetD/Math.max(artSize.z,.001),
            targetH/Math.max(artSize.y,.001)
          );
          art.scale.multiplyScalar(factor);
          art.updateMatrixWorld(true);
          artBox=new THREE.Box3().setFromObject(art);
          artSize=artBox.getSize(new THREE.Vector3());
          const artCenter=artBox.getCenter(new THREE.Vector3());
          const targetCenterY=caseBox.min.y + caseSize.y*.52;
          art.position.x += caseCenter.x-artCenter.x;
          art.position.z += caseCenter.z-artCenter.z;
          art.position.y += targetCenterY-artCenter.y;
          art.updateMatrixWorld(true);
          const finalBox=new THREE.Box3().setFromObject(art);
          const safeMinY=caseBox.min.y+caseSize.y*.22;
          const safeMaxY=caseBox.max.y-caseSize.y*.10;
          if(finalBox.min.y<safeMinY || finalBox.max.y>safeMaxY){
            art.scale.multiplyScalar(.88);
            art.updateMatrixWorld(true);
            const b2=new THREE.Box3().setFromObject(art);
            const c2=b2.getCenter(new THREE.Vector3());
            art.position.x += caseCenter.x-c2.x;
            art.position.z += caseCenter.z-c2.z;
            art.position.y += targetCenterY-c2.y;
            art.updateMatrixWorld(true);
          }
        }

        export function lockLeftExhibitContentsToCases(__gardenCtx){
          const THREE=__gardenCtx.THREE;
          const scene=__gardenCtx.scene;
          const pairs=[
          ...
          for(const prefix of pairs){
            const caseObj=museumPart(__gardenCtx,prefix,"case");
            const art=museumPart(__gardenCtx,prefix,"art");
            if(!caseObj) continue;
            scene.updateMatrixWorld(true);
            caseObj.updateMatrixWorld(true);
            const caseBox=new THREE.Box3().setFromObject(caseObj);
            if(caseBox.isEmpty()) continue;
            const caseCenter=caseBox.getCenter(new THREE.Vector3());
            if(art){
              const artWorld=new THREE.Vector3();
              art.getWorldPosition(artWorld);
              artWorld.x=caseCenter.x;
              art.position.copy(art.parent ? art.parent.worldToLocal(artWorld.clone()) : artWorld);
              art.updateMatrix();
              art.updateMatrixWorld(true);
            }
            const cloth=caseObj.parent?.getObjectByName(`${prefix}_purple_cloth`) || null;
            if(cloth){
              const clothWorld=new THREE.Vector3();
              cloth.getWorldPosition(clothWorld);
              clothWorld.x=caseCenter.x;
              cloth.position.copy(cloth.parent ? cloth.parent.worldToLocal(clothWorld.clone()) : clothWorld);
              cloth.updateMatrix();
        ...
        export function applyGardenConcreteColor(__gardenCtx){
          const style=__gardenCtx.GARDEN_CONCRETE_STYLE || GARDEN_CONCRETE_STYLE;
          const mat=style?.material;
          if(!mat) return;
          if(mat.userData?.gardenPavementGradient){
          ...
          mat.needsUpdate=true;
          const read=document.getElementById("gardenConcreteColorRead");
          if(read){
            read.textContent=
              `GARDEN CONCRETE COLOR\n#${style.color
                .toString(16)
                .padStart(6,"0")
                .toUpperCase()}`;
          }
        ...
        export function buildMuseumExhibit(__gardenCtx, displaySource,artSource,{name,caseHeight=4.25,title,subtitle}){
          ...
          const THREE=__gardenCtx.THREE;
          const applyMuseumGermanAlignmentExternal=__gardenCtx.applyMuseumGermanAlignment;
          ...
          fitArtworkIntoCase(__gardenCtx,art,display);
          ...
          queueMicrotask(()=>ensureDisplayCaseWoodenLegs(__gardenCtx,display));
          queueMicrotask(()=>refreshAllMuseumPurpleCloths(__gardenCtx));
          queueMicrotask(()=>applyDisplayCaseWoodColor(__gardenCtx));

          if(typeof applyMuseumGermanAlignmentExternal==="function"){
            applyMuseumGermanAlignmentExternal(false);
          }else{
            applyMuseumGermanAlignment(__gardenCtx,false);
          }
        ...
        export function loadGardenMuseumExhibits(__gardenCtx){
          ...
          loadGLBFromCandidates(
            ["./assets/models/display_case.glb"],
            displayGltf=>{
              const displaySource=prepareMuseumRoot(__gardenCtx,displayGltf.scene);
              removeDisplayCaseRedCloth(__gardenCtx,displaySource);
              const jobs=[
                {
                  files:["./assets/models/meteorite.glb"],
                ...
                {
                  files:["./assets/models/sculture1.glb"],
                ...
                {
                  files:["./assets/models/sculture2.glb"],
              ...
              jobs.forEach(job=>{
                loadGLBFromCandidates(job.files,gltf=>{
                  ...
                  if(typeof applyMuseumGermanAlignmentExternal==="function") applyMuseumGermanAlignmentExternal(true);
                  else applyMuseumGermanAlignment(__gardenCtx,true);
        ...
        export function refitMuseumArtwork(__gardenCtx, prefix){
          const {caseObj,art}=museumPrefixParts(__gardenCtx,prefix);
          if(!caseObj || !art) return;

          art.rotation.set(0,0,0);
          fitArtworkIntoCase(__gardenCtx,art,caseObj);
        ...
        export function enforceCenteredMuseumCasesAndBases(__gardenCtx){
          ...
          for(const o of [a1,a2,met]){
            ...
            ensureDisplayCaseWoodenLegs(__gardenCtx,o);
          }
        ...
        export function buildGardenFenceTreeBackdrop(__gardenCtx, source){
          ...
          const STATIC_GARDEN_TREE_LAYOUT=[
            ...
            [1,-45.000000000,0.032086425,94.000000000,12.825429699,0.145703191,11.175286206,16.031787125,11.687744542],
          ...
          // Trees are backdrop elements, so keep their crowns clear of the REAL
          // boundary fence, not an approximate x=100/z=190 line.
          const boundary=__gardenCtx.GARDEN_BOUNDARY_FENCE;
          const inward=Number.isFinite(boundary?.inward) ? boundary.inward : .08;
          const edit=boundary?.edit || {};
          const leftFenceX=-71.0+inward+(edit.left?.x || 0);
          const rightFenceX=71.0-inward+(edit.right?.x || 0);
          const farFenceZ=161.012-inward+(edit.back?.z || 0);

          // A little larger than before, but with extra crown-to-fence clearance.
          const TREE_SIZE_MULTIPLIER=1.12;

          // IMPORTANT: tree positions are now EXACTLY the authored layout positions.
          // No fence-based translation is applied anymore.
          const MANUAL_INSIDE_TREE_CENTERS=[
            {x:-44.400,z:94.000},
            {x:-52.400,z:132.000},
            {x:-54.700,z:95.700},
            {x:-56.800,z:117.700},
          ...
          const nearManualInsideTree=(x,z)=>{
            for(const p of MANUAL_INSIDE_TREE_CENTERS){
              const dx=x-p.x;
              const dz=z-p.z;
              if(dx*dx+dz*dz<=5.0*5.0) return true;
            }
            return false;
          };
          ...
          for(const [variant,x,y,z,targetH,rotY,sxN,syN,szN] of STATIC_GARDEN_TREE_LAYOUT){
            const actuallyInside=
              x>leftFenceX && x<rightFenceX &&
              z>56.900 && z<farFenceZ;

            const insideTarget=
              actuallyInside || nearManualInsideTree(x,z);

            // Only the smaller trees belonging to the inside group get bigger.
            // Outside/background trees keep their existing scale.
            const smallInside=insideTarget && targetH<15.0;
            const lowTreeBoost=smallInside ? 1.20 : 1.0;
            const lowTreeGirth=smallInside ? 1.18 : 1.0;
            ...
            const scale=new THREE.Vector3(
              (sxN*TREE_SIZE_MULTIPLIER*lowTreeGirth)/srcSize.y,
              (syN*TREE_SIZE_MULTIPLIER*lowTreeBoost)/srcSize.y,
              (szN*TREE_SIZE_MULTIPLIER*lowTreeGirth)/srcSize.y
            );
            ...
            GARDEN_TREE_DECOR.treeCollisionInstances.push({
              ...
              radius:trunkRadius,
              yMin:.025,
              yMax:.025+targetH*TREE_SIZE_MULTIPLIER*lowTreeBoost*1.25
            });
          }
        ...
        export function rebuildGardenWaterRings(__gardenCtx){
          ...
          GARDEN_FEATURES.waterDefs.forEach((def,i)=>{
            ...
            const mesh=new THREE.Mesh(geo,createRealisticWaterMaterial(__gardenCtx,def.opacity));
        ...
        export function loadApprovedGardenPalms(__gardenCtx){
          loadPalm(__gardenCtx,"./assets/models/date_palm.glb",{
            name:"date_palm_pair",
            type:"date",
            x:-48,
            z:63,
          ...
          loadPalm(__gardenCtx,"./assets/models/coconut_palm.glb",{
            name:"coconut_palm_pair",
            type:"coconut",
            x:-34,
            z:65,
            height:9.18,
        ...
        export function applyMuseumGermanAlignment(__gardenCtx, refresh=true){
          ...
          if(a1){
            ...
            refitMuseumArtwork(__gardenCtx,"garden_abstract_1_display_case");
            placeMuseumLabel(__gardenCtx,"garden_abstract_1_display_case");
          }
          if(a2){
            a2.position.set(MUSEUM_ALIGNMENT.leftX,MUSEUM_ALIGNMENT.caseY,museumLeftNearZ(__gardenCtx));
            a2.rotation.set(0,yaw,0);
            refitMuseumArtwork(__gardenCtx,"garden_abstract_2_display_case");
            placeMuseumLabel(__gardenCtx,"garden_abstract_2_display_case");
          }
          if(met){
            met.position.set(MUSEUM_ALIGNMENT.rightX,MUSEUM_ALIGNMENT.caseY,museumRightNearZ(__gardenCtx));
            met.rotation.set(0,yaw,0);
            refitMuseumArtwork(__gardenCtx,"garden_meteorite_display_case");
            placeMuseumLabel(__gardenCtx,"garden_meteorite_display_case");
          }
          ...
          if(a1) ensureDisplayCaseWoodenLegs(__gardenCtx,a1);
          if(a2) ensureDisplayCaseWoodenLegs(__gardenCtx,a2);
          if(met) ensureDisplayCaseWoodenLegs(__gardenCtx,met);

          enforceCenteredMuseumCasesAndBases(__gardenCtx);

          refreshAllMuseumPurpleCloths(__gardenCtx);
          lockLeftExhibitContentsToCases(__gardenCtx);
        ...
        export function setupApprovedGardenExhibitionLamps(__gardenCtx, source){
          const GARDEN_EXHIBITION_LAMPS=__gardenCtx.GARDEN_EXHIBITION_LAMPS;
          if(!source || !GARDEN_EXHIBITION_LAMPS) return [];

          GARDEN_EXHIBITION_LAMPS.lamps[0]=null;
          GARDEN_EXHIBITION_LAMPS.lamps[1]=null;

          const left=createGardenExhibitionLamp(
            __gardenCtx,
            source,
            0,
            -37.700,
          ...
          const right=createGardenExhibitionLamp(
            __gardenCtx,
            source,
            1,
            42.800,
          ...
          for(const lamp of [left,right]){
            ...
            lamp.traverse?.(obj=>{
              ...
              for(const material of materials){
                ...
                const darkMat=material.clone();

                if(darkMat.color){
                  darkMat.color.multiplyScalar(.42);
                }

                if(darkMat.emissive){
                  darkMat.emissive.multiplyScalar(.12);
                }

                if("emissiveIntensity" in darkMat){
                  darkMat.emissiveIntensity=
                    Math.min(
                      Number(darkMat.emissiveIntensity)||0,
                      .08
                    );
                }
        ...
        scene.add(gate);
              ...
              buildCeremonialGatePosts(__gardenCtx);
        ...
        export function gardenPavementFloorObjects(__gardenCtx){
          const found=[];
          const concreteMaterial=GARDEN_CONCRETE_STYLE.material;
          CEREMONY_GARDEN.group?.traverse?.(obj=>{
            if(!obj?.isMesh) return;
            const materials=Array.isArray(obj.material)?obj.material:[obj.material];
            if(
              concreteMaterial &&
              materials.some(material=>material===concreteMaterial)
            ){
              found.push(obj);
            }
          });
        ...
        export function applyGardenPavementHeight(__gardenCtx){
          const target=Number(GARDEN_PAVEMENT_EDITOR.height)||0;
          const delta=target-(Number(GARDEN_PAVEMENT_EDITOR.appliedHeight)||0);
          if(Math.abs(delta)<1e-9) return;
          for(const obj of gardenPavementFloorObjects()){
            obj.position.y+=delta;
            obj.updateMatrixWorld?.(true);
          }
          GARDEN_PAVEMENT_EDITOR.appliedHeight=target;
        }

        export function createDetailedBrownGatePost(__gardenCtx,name){
          const THREE=__gardenCtx.THREE;
          const CEREMONIAL_GATE_POSTS=__gardenCtx.CEREMONIAL_GATE_POSTS;
          ...
          const shaft=new THREE.Mesh(
            new THREE.BoxGeometry(.62,4.85,.62),
            wood
          );
          shaft.name="post_shaft";
          shaft.position.y=2.425;
          g.add(shaft);

          const base1=new THREE.Mesh(
            new THREE.BoxGeometry(
              CEREMONIAL_GATE_POSTS.bottomBase.width,
              CEREMONIAL_GATE_POSTS.bottomBase.height,
              CEREMONIAL_GATE_POSTS.bottomBase.depth
            ),
            darkWood
          );
          base1.name="post_bottom_dark_base";
          base1.position.y=CEREMONIAL_GATE_POSTS.bottomBase.y;
          g.add(base1);

          const base2=new THREE.Mesh(
            new THREE.BoxGeometry(.82,.22,.82),
            wood
          );
          base2.position.y=.39;
          g.add(base2);
          ...
          for(const side of [-1,1]){
            const insetX=new THREE.Mesh(
              new THREE.BoxGeometry(.08,3.75,.46),
              darkWood
            );
            insetX.position.set(side*.325,2.48,0);
            g.add(insetX);

            const insetZ=new THREE.Mesh(
              new THREE.BoxGeometry(.46,3.75,.08),
              darkWood
            );
            insetZ.position.set(0,2.48,side*.325);
            g.add(insetZ);
          }

          for(const y of [1.02,3.90,4.55]){
            const collar=new THREE.Mesh(
              new THREE.BoxGeometry(.82,.16,.82),
              bronze
            );
            collar.position.y=y;
            g.add(collar);
          ...
          const cap=new THREE.Mesh(
            new THREE.BoxGeometry(.96,.24,.96),
            darkWood
          );
          cap.position.y=4.94;
          g.add(cap);
          ...
          const crownLower=new THREE.Mesh(
            new THREE.BoxGeometry(.78,.18,.78),
            bronze
          );
          crownLower.position.y=5.13;
          g.add(crownLower);

          const crownTop=new THREE.Mesh(
            new THREE.BoxGeometry(.62,.16,.62),
            darkWood
          );
          crownTop.position.y=5.30;
          g.add(crownTop);
        ...
        export function buildCeremonialGatePosts(__gardenCtx){
          const scene=__gardenCtx.scene;
          const CEREMONIAL_GATE_POSTS=__gardenCtx.CEREMONIAL_GATE_POSTS;
          const refreshGardenObjectCollisions=__gardenCtx.refreshGardenObjectCollisions;
          const refreshCeremonialGatePostEditors=__gardenCtx.refreshCeremonialGatePostEditors;

          if(CEREMONIAL_GATE_POSTS.left?.parent){
            CEREMONIAL_GATE_POSTS.left.parent.remove(CEREMONIAL_GATE_POSTS.left);
          }
          if(CEREMONIAL_GATE_POSTS.right?.parent){
            CEREMONIAL_GATE_POSTS.right.parent.remove(CEREMONIAL_GATE_POSTS.right);
          }

          const left=createDetailedBrownGatePost(__gardenCtx,"ceremonial_gate_post_left");
          const right=createDetailedBrownGatePost(__gardenCtx,"ceremonial_gate_post_right");

          left.position.set(-4.250,0.020,59.930);
          right.position.set(5.750,0.020,59.930);
          left.rotation.set(0,0,0);
          right.rotation.set(0,0,0);
          left.scale.set(1.159,1.217,2.289);
          right.scale.set(1.159,1.217,2.289);

          scene.add(left,right);
          CEREMONIAL_GATE_POSTS.left=left;
          CEREMONIAL_GATE_POSTS.right=right;

          refreshGardenObjectCollisions?.();
          refreshCeremonialGatePostEditors?.();
        }

        export function createRealisticWaterMaterial(__gardenCtx,opacity){
          const THREE=__gardenCtx.THREE;
